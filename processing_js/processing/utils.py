...
class JsUtils:  # pylint: disable=too-many-public-methods
    ...
    def r_binary_folder() -> str:
        ...
        """
        folder = ProcessingConfig.getSetting(JsUtils.R_FOLDER)
        if not folder:
            folder = JsUtils.guess_r_binary_folder()
    ...
    def package_repo():
        ...
        """
        return ProcessingConfig.getSetting(JsUtils.R_REPO)
    ...
    def use_user_library():
        ...
        """
        return ProcessingConfig.getSetting(JsUtils.R_USE_USER_LIB)
    ...
    def r_library_folder():
        ...
        """
        folder = ProcessingConfig.getSetting(JsUtils.R_LIBS_USER)
        if folder is None:
    ...
    def get_process_encoding():
        ...
        """
        if JsUtils.is_windows():
            return "cp{}".format(JsUtils.get_windows_code_page())
    ...
    def execute_r_algorithm(alg, parameters, context, feedback):
        ...
        script_filename = JsUtils.create_r_script_from_commands(alg.build_r_script(parameters, context, feedback))

        # run commands
        command = [
            JsUtils.path_to_r_executable(script_executable=True),
            script_filename
        ]

        feedback.pushInfo(JsUtils.tr('R execution console output'))
        ...
        with subprocess.Popen(command,
                              ...
                              encoding=JsUtils.get_process_encoding(),
                              startupinfo=JsUtils.get_process_startup_info(),
                              universal_newlines=True) as proc:
            for line in iter(proc.stdout.readline, ''):
                if feedback.isCanceled():
                    proc.terminate()

                if JsUtils.is_error_line(line):
    ...
    def path_to_r_executable(script_executable=False) -> str:
        ...
        bin_folder = JsUtils.r_binary_folder()
        if bin_folder:
            if JsUtils.is_windows():
                if ProcessingConfig.getSetting(JsUtils.R_USE64):
                    exec_dir = 'x64'
                else:
                    exec_dir = 'i386'
    ...
    def check_r_is_installed() -> Optional[str]:
        ...
        """
        if DEBUG:
            QgsMessageLog.logMessage(JsUtils.tr('R binary path: {}').format(JsUtils.path_to_r_executable()), 'R',
                                     Qgis.Info)

        if JsUtils.is_windows():
            path = JsUtils.r_binary_folder()
            if path == '':
                return JsUtils.tr('R folder is not configured.\nPlease configure '
                                 'it before running R scripts.')

        command = [JsUtils.path_to_r_executable(), '--version']
        try:
            with subprocess.Popen(command,
                                  stdout=subprocess.PIPE,
                                  stdin=subprocess.DEVNULL,
